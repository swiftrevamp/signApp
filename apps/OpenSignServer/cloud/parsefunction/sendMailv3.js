    ...
    if (req.params.url) {
      ...
      try {
        ...
        if (ress) {
          ...
          // CHANGE START: Removed req.params.from usage
          const mailsender = smtpenable ? process.env.SMTP_USER_EMAIL : process.env.MAILGUN_SENDER;
          const replyto = req.params?.replyto || '';
          
          const messageParams = {
            from: mailsender, // ✅ CHANGE: Sirf 'noreply' email use hoga, purana naam hat gaya
            to: req.params.recipient,
            subject: req.params.subject,
            text: req.params.text || 'mail',
            html: req.params.html || '',
      ...
      // CHANGE START: Removed req.params.from usage in else block too
      const mailsender = smtpenable ? process.env.SMTP_USER_EMAIL : process.env.MAILGUN_SENDER;
      const replyto = req.params?.replyto || '';
      
      const messageParams = {
        from: mailsender, // ✅ CHANGE: Sirf 'noreply' email use hoga
        to: req.params.recipient,
        subject: req.params.subject,
        text: req.params.text || 'mail',
        html: req.params.html || '',
